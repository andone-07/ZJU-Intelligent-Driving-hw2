...
if __name__ == '__main__':
    # 加载模型
    model = YOLO(r'cfg/models/v8/yolov8.yaml')  # 不使用预训练权重训练
    # model = YOLO(r'yolov8.yaml').load("yolov8n.pt")  # 使用预训练权重训练
    # 训练参数 ----------------------------------------------------------------------------------------------
    model.train(
        data=r'D:/Desktop/ultralytics-main/ultralytics/cfg/datasets/coco.yaml',
        epochs=102,  # (int) 训练的周期数
        patience=50,  # (int) 等待无明显改善以进行早期停止的周tiao期数
        batch=8,  # (int) 每批次的图像数量（-1 为自动批处理）
        imgsz=1920,  # (int) 输入图像的大小，整数或w，h
